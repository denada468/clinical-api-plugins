...
class SampleApp < Sinatra::Base
  ...
  get "/refresh-jwt" do
    @jwt_hit="refresh_jwt was called meh"
    refresh_jwt
  end
  ...
  get "/protected-refresh-jwt" do
    if logged_in?
      @jwt_hit="refresh_jwt was called via /protected-refresh-jwt function"
      refresh_jwt
    else
      "not logged in"
      @jwt_hit="refresh_jwt wasn't called bc we are not logged in"
    end
  end
  
  def refresh_jwt
    @jwt_hit="in jwt function"
    @uri = URI("#{CONFIG[:api_server]}/v1/tokens")
    @api_key = CONFIG[:api_key]
